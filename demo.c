...
int main(int argc, char** argv) {
    ...
    if (argc < 2) {
        ...
        printf("Please drag and drop the image file onto the executable file, or use the command "
               "line: demo.exe image \n ");
    ...
    if ((Channels != 0) && (Width != 0) && (Height != 0)) {
        ...
        (unsigned char*)stbi__malloc(Width * Channels * Height * sizeof(unsigned char));
        if (inputImage) {
