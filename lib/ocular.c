...
#include "util.h"
    ...
    OC_STATUS ocularFilmNoirEffect(unsigned char* Input, unsigned char* Output, int Width, int Height, int Stride, float shadowCutoff,
        ...
        /* Algorithm ported from the PhotoDemon project
         * Copyright 2024 by Tanner Helland.
        ...
        float shadowStrength = shadowCutoff;
        float contrastStrength = contrastBoost;

        // Shadow and highlight strength are on the range 0-100.
        // Invert highlight so it's on the range [155, 255]
        float highlightStrength = 255 - highlightCutoff;

        // Given the distance between shadow and highlight values, determine the remap value on a [0, 100] scale.
        float contrastRange = highlightStrength - shadowStrength;
        contrastRange = (255 / contrastRange);

        // The luminance midpoint is on the range [0, 100], with a default of 50. Remap it to [0, 255]
        float luminancePoint = contrastMidpoint * 2.55;

        // Film grain is on the range [0, 100]. Remap it to [-50, 50].
        float artificialGrain = grainPercentage - 50;

        if (artificialGrain > 0) {
            srand(time(NULL));
        }
        ...
        for (int y = 0; y < Height; y++) {
            ...
            for (int x = 0; x < Width; x++) {
                ...
                gray = (218 * pInput[0] + 732 * pInput[1] + 74 * pInput[2]) / 1024;

                gray = (gray - shadowStrength) * contrastRange;

                gray = gray + (((gray - luminancePoint) * contrastStrength) / 100);

                if (artificialGrain > 0) {
                    gray = gray + (artificialGrain * rand());
                }

                grayByte = ClampToByte(gray);
