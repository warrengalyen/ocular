...
#include "util.h"
    ...
    // Helper function for CLAHE to handle histogram clipping, redistribution, and mapping.
    static void ProcessHistogram(int* histogram, int* mapping, int totalPixels, float cutLimitValue, float adaptValue, 
                                 bool classicEqualization) {
        if (classicEqualization) {
            // Classic histogram equalization
            int sum = 0;
            for (int i = 0; i < 256; i++) {
                sum += histogram[i];
                mapping[i] = (sum * 255) / (float)totalPixels;
            }
            return;
        }
        
        int clipLimit = (int)(cutLimitValue * (float)totalPixels / 256.0f);
        ...
        for (int i = 0; i < 256; i++) {
            if (histogram[i] > clipLimit) {
                redistBatch += histogram[i] - clipLimit;
                histogram[i] = clipLimit;
            }
        }
        
        // Redistribute clipped pixels
        int redistStep = redistBatch / 256;
        int redistRemainder = redistBatch % 256;
        for (int i = 0; i < 256; i++) {
            histogram[i] += redistStep;
            if (i < redistRemainder) {
                histogram[i]++;
            }
        }
        ...
        // Apply adapation curve
        if (adaptValue < 1.0f) {
            for (int i = 0; i < 256; i++) {
                float normalized = i / 255.0f;
                float blend = adaptValue + (1.0f - adaptValue) * (1.0f - 4.0f * (normalized - 0.5f) * (normalized - 0.5f));
                if (blend < 0.0f)
                    blend = 0.0f;
                if (blend > 1.0f)
                    blend = 1.0f;

                mapping[i] = i + (mapping[i] - i) * blend;
            }
        }
    }

    OC_STATUS ocularCLAHEFilter(unsigned char* Input, unsigned char* Output, int Width, int Height, int Stride, int colorCorrection,
                                int Adaptation, int CutLimit, bool ClassicEqualization, int chromaDampen) {
        if (Input == NULL || Output == NULL)
            return OC_STATUS_ERR_NULLREFERENCE;
        if (Width <= 0 || Height <= 0 || Stride <= 0)
            return OC_STATUS_ERR_INVALIDPARAMETER;
        ...
        /* Validate and normalize parameters */
        colorCorrection = clamp(colorCorrection, 0, 100);
        Adaptation = clamp(Adaptation, 0, 100);
        CutLimit = clamp(CutLimit, 0, 20);
        chromaDampen = clamp(chromaDampen, 0, 100);

        /* Convert percentage values to actual parameters */
        float adaptValue = Adaptation / 100.0f;
        float cutLimitValue = CutLimit / 100.0f;
        float dampStrength = chromaDampen / 100.0f; /* 0..1 */

        if (Channels >= 3) {
            size_t npx = (size_t)Width * (size_t)Height;

            /* Process in LAB color space for better color preservation */
            double* LChannel = (double*)malloc(npx * sizeof(double));
            double* aChannel = (double*)malloc(npx * sizeof(double));
            double* bChannel = (double*)malloc(npx * sizeof(double));

            if (!LChannel || !aChannel || !bChannel) {
                free(LChannel);
                free(aChannel);
                free(bChannel);
                return OC_STATUS_ERR_OUTOFMEMORY;
            ...
            for (int y = 0; y < Height; y++) {
                for (int x = 0; x < Width; x++) {
                    int idx = y * Stride + x * Channels;
                    int labIdx = y * Width + x;

                    double L, a, b;
                    rgb2lab(Input[idx], Input[idx + 1], Input[idx + 2], &L, &a, &b);
                    LChannel[labIdx] = L;
                    aChannel[labIdx] = a;
                    bChannel[labIdx] = b;
                }
            ...
            /* Build histogram from L channel (rounded binning, 0..100 -> 0..255) */
            int histogram[256] = { 0 };
            int mapping[256] = { 0 };

            for (size_t i = 0; i < npx; i++) {
                int binValue = (int)(LChannel[i] * (255.0 / 100.0) + 0.5);
                if (binValue < 0)
                    binValue = 0;
                if (binValue > 255)
                    binValue = 255;
            ...
            /* Use your existing helper exactly as-is */
            ProcessHistogram(histogram, mapping, (int)npx, cutLimitValue, adaptValue, ClassicEqualization);

            /* Apply mapping to L channel (and optionally dampen chroma) */
            for (size_t i = 0; i < npx; i++) {
                double oldL = LChannel[i];

                int binValue = (int)(oldL * (255.0 / 100.0) + 0.5);
                if (binValue < 0)
                    binValue = 0;
                if (binValue > 255)
                    binValue = 255;

                /* mapping[] is 0..255 -> L is 0..100 */
                double newL = (double)mapping[binValue] * (100.0 / 255.0);
                newL = clamp(newL, 0.0, 100.0);

                if (dampStrength > 0.0f) {
                    /* Dampen chroma to avoid blotchy shadow colors from out-of-gamut clipping:
                       - strongest in deep shadows
                       - only applies when darkening (newL < oldL) */
                    double kBase = 1.0;

                    if (newL < oldL) {
                        double denom = (oldL > 1e-6) ? oldL : 1e-6;
                        kBase = newL / denom; /* 0..1 */
                        kBase = clamp(kBase, 0.0, 1.0);
                    }

                    /* Additional shadow weighting: stronger near black */
                    double shadow = newL / 100.0; /* 0..1 */
                    double ks = shadow * shadow;  /* quadratic falloff */
                    kBase *= ks;                  /* 0..1 */

                    /* Blend toward dampening based on user strength */
                    double k = 1.0 + (kBase - 1.0) * (double)dampStrength;
            ...
            for (int y = 0; y < Height; y++) {
                for (int x = 0; x < Width; x++) {
                    ...
                    unsigned char r, g, b;
                    double L = clamp(LChannel[labIdx], 0.0, 100.0);
                    lab2rgb(L, aChannel[labIdx], bChannel[labIdx], &r, &g, &b);
            ...
            if (Channels == 4) {
                for (int y = 0; y < Height; y++) {
                    for (int x = 0; x < Width; x++) {
                        Output[y * Stride + x * Channels + 3] = Input[y * Stride + x * Channels + 3];
                    }
                }
            }
            ...
            if (colorCorrection > 0) {
                float correction = colorCorrection / 100.0f;
                for (int y = 0; y < Height; y++) {
                    for (int x = 0; x < Width; x++) {
                        int idx = y * Stride + x * Channels;
                        float avg = (Output[idx] + Output[idx + 1] + Output[idx + 2]) / 3.0f;
                        for (int c = 0; c < 3; c++) {
                            float diff = Output[idx + c] - avg;
                            Output[idx + c] = ClampToByte(avg + diff * correction);
                        }
                    }
                }
            }
            ...
            for (int y = 0; y < Height; y++) {
                for (int x = 0; x < Width; x++) {
                    histogram[Input[y * Stride + x]]++;
                }
            }

            ProcessHistogram(histogram, mapping, Width * Height, cutLimitValue, adaptValue, ClassicEqualization);

            for (int y = 0; y < Height; y++) {
                for (int x = 0; x < Width; x++) {
                    Output[y * Stride + x] = (unsigned char)mapping[Input[y * Stride + x]];
                }
            }
