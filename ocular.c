...
float dot(unsigned char R, unsigned char G, unsigned char B, float fR, float fG,
          float fB) {
  return (float)(R * fR + G * fG + B * fB);
}

static inline float mix(float a, float b, float alpha) {
  return (a * (1.0f - alpha) + b * alpha);
}

unsigned char degree(unsigned char InputColor, unsigned char OutputColor,
                     float intensity) {
  return (unsigned char)ClampToByte(
      ((intensity * OutputColor) + (1.0f - intensity) * InputColor));
}
...
void ocularGrayscaleFilter(unsigned char *Input, unsigned char *Output,
  ...
  if (Channels == 3) {
    for (int Y = 0; Y < Height; Y++) {
      ...
      for (; X < Width - 4; X += 4, LinePS += Channels * 4) {
        LinePD[X + 0] =
            (B_WT * LinePS[0] + G_WT * LinePS[1] + R_WT * LinePS[2]) >> 8;
        LinePD[X + 1] =
            (B_WT * LinePS[3] + G_WT * LinePS[4] + R_WT * LinePS[5]) >> 8;
        LinePD[X + 2] =
            (B_WT * LinePS[6] + G_WT * LinePS[7] + R_WT * LinePS[8]) >> 8;
        LinePD[X + 3] =
            (B_WT * LinePS[9] + G_WT * LinePS[10] + R_WT * LinePS[11]) >> 8;
      }
    ...
    for (int Y = 0; Y < Height; Y++) {
      ...
      for (; X < Width - 4; X += 4, LinePS += Channels * 4) {
        LinePD[X + 0] =
            (B_WT * LinePS[0] + G_WT * LinePS[1] + R_WT * LinePS[2]) >> 8;
        LinePD[X + 1] =
            (B_WT * LinePS[4] + G_WT * LinePS[5] + R_WT * LinePS[6]) >> 8;
        LinePD[X + 2] =
            (B_WT * LinePS[8] + G_WT * LinePS[9] + R_WT * LinePS[10]) >> 8;
        LinePD[X + 3] =
            (B_WT * LinePS[12] + G_WT * LinePS[13] + R_WT * LinePS[14]) >> 8;
      }
      for (; X < Width; X++, LinePS += Channels) {
        LinePD[X] =
            (B_WT * LinePS[0] + G_WT * LinePS[1] + R_WT * LinePS[2]) >> 8;
...
void ocularRGBFilter(unsigned char *Input, unsigned char *Output, int Width,
                     int Height, int Stride, float redAdjustment,
...
void ocularAverageLuminanceThresholdFilter(unsigned char *Input,
  ...
  if (Channels == 1) {
    ...
    Luminance =
        (unsigned char)((SumPix / numberOfPixels) * thresholdMultiplier);
...
void ocularAverageColor(unsigned char *Input, int Width, int Height, int Stride,
                        unsigned char *AverageR, unsigned char *AverageG,
...
void ocularLuminosity(unsigned char *Input, int Width, int Height, int Stride,
                      unsigned char *Luminance) {
...
void ocularColorMatrixFilter(unsigned char *Input, unsigned char *Output,
                             int Width, int Height, int Stride,
  ...
  if (Channels == 4) {
    ...
    for (int Y = 0; Y < Height; Y++) {
      ...
      for (int X = 0; X < Width; X++) {
        ...
        outB = ClampToByte(colorMatrix8[r] + colorMatrix9[g] +
                           colorMatrix10[b] + colorMatrix11[a]);
        unsigned char *pDegreeMapB = degreeMap + (b << 8);
        pOutput[2] = pDegreeMapB[outB];
        outA = ClampToByte(colorMatrix12[r] + colorMatrix13[g] +
                           colorMatrix14[b] + colorMatrix15[a]);
    ...
    for (int Y = 0; Y < Height; Y++) {
      ...
      for (int X = 0; X < Width; X++) {
        ...
        pOutput[1] = pDegreeMapG[outG];
        outB =
            ClampToByte(colorMatrix8[r] + colorMatrix9[g] + colorMatrix10[b]);
        unsigned char *pDegreeMapB = degreeMap + (b << 8);
        pOutput[2] = pDegreeMapB[outB];
        pInput += Channels;
        pOutput += Channels;
...
void ocularSepiaFilter(unsigned char *Input, unsigned char *Output, int Width,
  ...
  float colorMatrix[4 * 4] = {
      0.3588f, 0.7044f, 0.1368f, 0.0f, 0.2990f, 0.5870f, 0.1140f, 0.0f,
      0.2392f, 0.4696f, 0.0912f, 0.0f, 0.f,     0.f,     0.f,     1.f};
  ocularColorMatrixFilter(Input, Output, Width, Height, Stride, colorMatrix,
                          fIntensity);
}

void ocularChromaKeyFilter(unsigned char *Input, unsigned char *Output,
                           int Width, int Height, int Stride,
                           unsigned char colorToReplaceR,
                           unsigned char colorToReplaceG,
                           unsigned char colorToReplaceB,
  ...
  unsigned char maskY = (19589 * colorToReplaceR + 38443 * colorToReplaceG +
                         7504 * colorToReplaceB) >>
                        16;
  ...
  if (Channels == 3) {
    ...
    for (int Cr = 0; Cr < 256; Cr++) {
      ...
      for (int Cb = 0; Cb < 256; Cb++) {
        pBlendMap[Cb] =
            (short)(255.0f *
                    smoothstep((float)iThresholdSensitivity,
                               (float)iThresholdSensitivity + iSmoothing,
                               vec2_distance((float)Cr, (float)Cb,
                                             (float)maskCr, (float)maskCb)));
      }
    }
    for (int Y = 0; Y < Height; Y++) {
      unsigned char *pOutput = Output + (Y * Stride);
...
void ocularLookupFilter(unsigned char *Input, unsigned char *Output,
                        unsigned char *lookupTable, int Width, int Height,
  ...
  for (int b = 0; b < 256; b++) {
    ...
    fractMap[b] =
        (unsigned short)(256 * (pixelColorMap[b] - truncf(pixelColorMap[b])));
  ...
  for (int Y = 0; Y < Height; Y++) {
    ...
    for (int X = 0; X < Width; X++) {
      ...
      unsigned char *pLineLookup1 =
          &lookupTable[(((int)(quad1yMap[B] + green) << 9) +
                        (int)(quad1xMap[B] + red)) *
                       lookupChannels];
      unsigned char *pLineLookup2 =
          &lookupTable[(((int)(quad2yMap[B] + green) << 9) +
                        (int)(quad2xMap[B] + red)) *
                       lookupChannels];
      unsigned short fractB = fractMap[B];
      pOutput[0] = (int)(R * c1 + ((*pLineLookup1++ * (256 - fractB) +
                                    *pLineLookup2++ * fractB) >>
                                   8) *
                                      c2) >>
                   8;
      pOutput[1] = (int)(G * c1 + ((*pLineLookup1++ * (256 - fractB) +
                                    *pLineLookup2++ * fractB) >>
                                   8) *
                                      c2) >>
                   8;
      pOutput[2] = (int)(B * c1 + ((*pLineLookup1++ * (256 - fractB) +
                                    *pLineLookup2++ * fractB) >>
                                   8) *
                                      c2) >>
                   8;
...
void ocularSaturationFilter(unsigned char *Input, unsigned char *Output,
                            int Width, int Height, int Stride,
                            float saturation) {
  ...
  for (int grey = 0; grey < 256; grey++) {
    ...
    for (int Input = 0; Input < 256; Input++) {
      pSaturationMap[0] =
          (unsigned char)((mix_u8(grey, Input, saturation) + Input) * 0.5f);
      pSaturationMap++;
    }
  }
...
void ocularGammaFilter(unsigned char *Input, unsigned char *Output, int Width,
                       int Height, int Stride, float gamma) {
...
void ocularConstrastFilter(unsigned char *Input, unsigned char *Output,
                           int Width, int Height, int Stride, float contrast) {
...
void ocularExposureFilter(unsigned char *Input, unsigned char *Output,
                          int Width, int Height, int Stride, float exposure) {
...
void ocularBrightnessFilter(unsigned char *Input, unsigned char *Output,
                            int Width, int Height, int Stride, int brightness) {
...
void ocularHazeFilter(unsigned char *Input, unsigned char *Output, int Width,
                      int Height, int Stride, float distance, float slope,
                      int intensity) {
  ...
  for (int Y = 0; Y < Height; Y++) {
    ...
    unsigned char *pInput = Input + (Y * Stride);
    for (int X = 0; X < Width; X++) {
      pOutput[0] = (int)(pInput[0] * c1 +
                         ((ClampToByte(pInput[0] - distanceColorMap[Y]) *
                           patchDistanceMap[Y]) >>
                          8) *
                             c2) >>
                   8;
      pOutput[1] = (int)(pInput[1] * c1 +
                         ((ClampToByte(pInput[1] - distanceColorMap[Y]) *
                           patchDistanceMap[Y]) >>
                          8) *
                             c2) >>
                   8;
      pOutput[2] = (int)(pInput[2] * c1 +
                         ((ClampToByte(pInput[2] - distanceColorMap[Y]) *
                           patchDistanceMap[Y]) >>
                          8) *
                             c2) >>
                   8;
...
void ocularOpacityFilter(unsigned char *Input, unsigned char *Output, int Width,
                         int Height, int Stride, float opacity) {
...
void ocularLevelsFilter(unsigned char *Input, unsigned char *Output, int Width,
                        int Height, int Stride,
                        ocularLevelParams *redLevelParams,
                        ocularLevelParams *greenLevelParams,
  ...
  for (int pixel = 0; pixel < 256; pixel++) {
    if (redLevelParams->Enable) {
      LevelMapR[pixel] = (mix_u8(
          redLevelParams->minOutput, redLevelParams->maxOutput,
          (powf(min(max(pixel - redLevelParams->levelMinimum, (0.0f)) /
                        (redLevelParams->levelMaximum -
                         redLevelParams->levelMinimum),
                    (255)),
    ...
    if (greenLevelParams->Enable) {
      LevelMapG[pixel] = (mix_u8(
          greenLevelParams->minOutput, greenLevelParams->maxOutput,
          (powf(min(max(pixel - greenLevelParams->levelMinimum, (0.0f)) /
                        (greenLevelParams->levelMaximum -
                         greenLevelParams->levelMinimum),
                    (255)),
    ...
    if (blueLevelParams->Enable) {
      LevelMapB[pixel] = (mix_u8(
          blueLevelParams->minOutput, blueLevelParams->maxOutput,
          (powf(min(max(pixel - blueLevelParams->levelMinimum, (0.0f)) /
                        (blueLevelParams->levelMaximum -
                         blueLevelParams->levelMinimum),
                    (255)),
...
void rgb2yiq(unsigned char *R, unsigned char *G, unsigned char *B, short *Y,
             short *I, short *Q) {
  *Y = ((int)(0.299f * 65536) * *R + (int)(0.587f * 65536) * *G +
        (int)(0.114f * 65536) * *B) >>
       16;
  *I = ((int)(0.595f * 65536) * *R - (int)(0.274453f * 65536) * *G -
        (int)(0.321263f * 65536) * *B) >>
       16;
  *Q = ((int)(0.211456f * 65536) * *R - (int)(0.522591f * 65536) * *G +
        (int)(0.311135f * 65536) * *B) >>
       16;
}

void yiq2rgb(short *Y, short *I, short *Q, unsigned char *R, unsigned char *G,
             unsigned char *B) {
  *R = ClampToByte((int)(Y + ((((int)(0.9563 * 65536)) * (*I)) +
                              ((int)(0.6210 * 65536)) * (*Q))) >>
                   16);
  *G = ClampToByte((int)(Y - ((((int)(0.2721 * 65536)) * (*I)) +
                              ((int)(0.6474 * 65536)) * (*Q))) >>
                   16);
  *B = ClampToByte((int)(Y + ((((int)(1.7046 * 65536)) * (*Q)) -
                              ((int)(1.1070 * 65536)) * (*I))) >>
                   16);
}

void ocularHueFilter(unsigned char *Input, unsigned char *Output, int Width,
                     int Height, int Stride, float hueAdjust) {
  ...
  for (int Q = 0; Q < 256; Q++) {
    ...
    for (int I = 0; I < 256; I++) {
      pChromaMap[0] =
          sqrtf((float)((I - 128) * (I - 128) + (Q - 128) * (Q - 128)));
      pChromaMap++;
    }
  }
  for (int Q = 0; Q < 256; Q++) {
...
void ocularHighlightShadowTintFilter(
    unsigned char *Input, unsigned char *Output, int Width, int Height,
  ...
  for (int lum = 0; lum < 256; lum++) {
    ...
    for (int pixel = 0; pixel < 256; pixel++) {
      ...
      float shadowResultR =
          mix(fpixel, max(fpixel, mix(shadowTintR, fpixel, luminance)),
              shadowTintIntensity);
      float shadowResultG =
          mix(fpixel, max(fpixel, mix(shadowTintG, fpixel, luminance)),
              shadowTintIntensity);
      float shadowResultB =
          mix(fpixel, max(fpixel, mix(shadowTintB, fpixel, luminance)),
              shadowTintIntensity);
      float highlightResultR =
          mix(fpixel,
              min(shadowResultR, mix(shadowResultR, highlightTintR, luminance)),
              highlightTintIntensity);
      float highlightResultG =
          mix(fpixel,
              min(shadowResultG, mix(shadowResultG, highlightTintG, luminance)),
              highlightTintIntensity);
      float highlightResultB =
          mix(fpixel,
              min(shadowResultB, mix(shadowResultB, highlightTintB, luminance)),
              highlightTintIntensity);
...
void ocularHighlightShadowFilter(unsigned char *Input, unsigned char *Output,
                                 int Width, int Height, int Stride,
        ...
        (short)(255.0f *
                clamp((1.0f -
                       (powf(1.0f - luminance, 1.0f / (2.0f - highlights)) +
                        (-0.8f) * powf(1.0f - luminance,
                                       2.0f / (2.0f - highlights)))) -
                          luminance,
                      -1.0f, 0.0f));
  }
  ...
  for (int Y = 0; Y < Height; Y++) {
    ...
    for (int X = 0; X < Width; X++) {
      const short luminance = luminanceWeightingMap[pInput[0]] +
                              luminanceWeightingMap[pInput[1]] +
...
void ocularMonochromeFilter(unsigned char *Input, unsigned char *Output,
                            int Width, int Height, int Stride,
                            unsigned char filterColorR,
  ...
  for (int luminance = 0; luminance < 256; luminance++) {
    ...
    filterColorRMap[luminance] =
        (unsigned char)(255.0f * (lum < 0.5f ? (2.0f * lum * fColorR)
                                             : (1.0f - 2.0f * (1.0f - lum) *
                                                           (1.0f - fColorR))));
    filterColorGMap[luminance] =
        (unsigned char)(255.0f * (lum < 0.5f ? (2.0f * lum * fColorG)
                                             : (1.0f - 2.0f * (1.0f - lum) *
                                                           (1.0f - fColorG))));
    filterColorBMap[luminance] =
        (unsigned char)(255.0f * (lum < 0.5f ? (2.0f * lum * fColorB)
                                             : (1.0f - 2.0f * (1.0f - lum) *
                                                           (1.0f - fColorB))));
  }
  for (int Y = 0; Y < Height; Y++) {
...
void ocularColorInvertFilter(unsigned char *Input, unsigned char *Output,
                             int Width, int Height, int Stride) {
...
void ocularSolidColorGenerator(unsigned char *Output, int Width, int Height,
                               int Stride, unsigned char colorR,
...
void ocularLuminanceThresholdFilter(unsigned char *Input, unsigned char *Output,
                                    int Width, int Height, int Stride,
...
void ocularWhiteBalanceFilter(unsigned char *Input, unsigned char *Output,
                              int Width, int Height, int Stride,
  ...
  for (int pixel = 0; pixel < 256; pixel++) {
    float fpixel = pixel * (1.0f / 255.0f);
    QTint[pixel] =
        (short)clamp((float)(pixel - 127 + plusTint), -127.0f, 127.0f);
    float processedR =
        (fpixel < 0.5f
             ? (2.0f * fpixel * warmFilterR)
             : (1.0f - 2.0f * (1.0f - fpixel) * (1.0f - warmFilterR)));
    float processedG =
        (fpixel < 0.5f
             ? (2.0f * fpixel * warmFilterG)
             : (1.0f - 2.0f * (1.0f - fpixel) * (1.0f - warmFilterG)));
    float processedB =
        (fpixel < 0.5f
             ? (2.0f * fpixel * warmFilterB)
             : (1.0f - 2.0f * (1.0f - fpixel) * (1.0f - warmFilterB)));
    processedRMap[pixel] =
        ClampToByte(255.0f * mix(fpixel, processedR, Temperature));
    processedGMap[pixel] =
        ClampToByte(255.0f * mix(fpixel, processedG, Temperature));
    processedBMap[pixel] =
        ClampToByte(255.0f * mix(fpixel, processedB, Temperature));
  }
  for (int Y = 0; Y < Height; Y++) {
    unsigned char *pOutput = Output + (Y * Stride);
    unsigned char *pInput = Input + (Y * Stride);
    for (int X = 0; X < Width; X++) {
      ...
      YPrime = ((int)(0.299f * 65536) * R + (int)(0.587f * 65536) * G +
                (int)(0.114f * 65536) * B) >>
               16;
      I = ((int)(0.595f * 65536) * R - (int)(0.274453f * 65536) * G -
           (int)(0.321263f * 65536) * B) >>
      ...
      pOutput[0] = processedRMap[ClampToByte(
          YPrime +
          (((int)(0.9563 * 65536) * I + (int)(0.6210 * 65536) * Q) >> 16))];
      pOutput[1] = processedGMap[ClampToByte(
          YPrime -
          (((int)(0.2721 * 65536) * I + (int)(0.6474 * 65536) * Q) >> 16))];
      pOutput[2] = processedBMap[ClampToByte(
          YPrime +
          (((int)(1.7046 * 65536) * Q - (int)(1.1070 * 65536) * I) >> 16))];
...
void ocularVibranceFilter(unsigned char *Input, unsigned char *Output,
                          int Width, int Height, int Stride, float vibrance) {
...
void rgb2hsv(const unsigned char *R, const unsigned char *G,
             const unsigned char *B, unsigned char *H, unsigned char *S,
             unsigned char *V) {
  int r = *R;
  int g = *G;
  int b = *B;
...
void hsv2rgb(const unsigned char *H, const unsigned char *S,
...
void ocularSkinToneFilter(unsigned char *Input, unsigned char *Output,
                          int Width, int Height, int Stride,
                          float skinToneAdjust, float skinHue,
                          float skinHueThreshold, float maxHueShift,
...
void gaussianHorizontal(unsigned char *bufferPerLine,
                        ...
                        int width, int height, int Channels, int Nwidth,
...
void ocularGaussianBlurFilter(unsigned char *Input, unsigned char *Output,
                              int Width, int Height, int Stride,
                              float GaussianSigma) {
  int Channels = Stride / Width;
  ...
  for (int y = 0; y < Height; ++y) {
    ...
    gaussianHorizontal(bufferPerLine, lpRowInitial, lpColInitial, Width, Height,
                       Channels, Width, a0a1, a2a3, b1b2, cprev, cnext);
  }
...
void ocularUnsharpMaskFilter(unsigned char *Input, unsigned char *Output,
                             int Width, int Height, int Stride,
  ...
  for (unsigned int PS = 0; PS < 256; PS++) {
    ...
    for (unsigned int PD = 0; PD < 256; PD++) {
      ...
      retPD = ((PS <= 128) ? (retPD * PS / 128)
                           : (255 - (255 - retPD) * (255 - PS) / 128));
  ...
  case 3: {
    ...
    for (int Y = 0; Y < Height; Y++) {
      ...
      for (int X = 0; X < Width; X++) {
        pTemp[0] = (unsigned char)((19595 * pInput[0] + 38470 * pInput[1] +
                                    7471 * pInput[2]) >>
                                   16);
    ...
    for (int Y = 0; Y < Height; Y++) {
      ...
      unsigned char *pBlur = Blur + (Y * Width);
      for (int x = 0; x < Width; x++) {
        cb = (unsigned char)((36962 * (pInput[2] - (int)(pTemp[0])) >> 16) +
                             128);
        cr = (unsigned char)((46727 * (pInput[0] - (int)(pTemp[0])) >> 16) +
                             128);
        // Sharpen: High Contrast Overlay

        unsigned char *pUnsharpMaskMap = unsharpMaskMap + (pTemp[0] << 8);

        ycbcr2rgb(pUnsharpMaskMap[pBlur[0]], cb, cr, &pOutput[0], &pOutput[1],
                  &pOutput[2]);
...
static inline void boxfilterRow(unsigned char *Input, unsigned char *Output,
...
static inline void boxfilterCol(unsigned char *Input, unsigned char *Output,
                                int Width, int Height, int Channels,
                                int Radius) {
  int iScale = (int)((256.0f * 256.0f) / (2 * Radius + 1));
  int iWidthStep = Width * Channels;
...
void ocularBoxBlurFilter(unsigned char *Input, unsigned char *Output, int Width,
                         int Height, int Stride, int Radius) {
...
void ocularSharpenFilter(unsigned char *Input, unsigned char *Output, int Width,
                         int Height, int Stride, float Radius, int sharpness,
                         int intensity) {
  ...
  case 3: {
    ...
    for (int Y = 0; Y < Height; Y++) {
      ...
      for (int x = 0; x < Width; x++) {
        ...
        ycbcr2rgb(pSharpnessMap[pBlur[0]], cb, cr, &pOutput[0], &pOutput[1],
                  &pOutput[2]);
...
void ocularResamplingFilter(unsigned char *Input, unsigned int Width,
                            unsigned int Height, unsigned int Stride,
                            unsigned char *Output, int newWidth, int newHeight,
                            int dstStride) {
  ...
  for (int y = 0; y < newHeight; y++) {
    ...
    for (int x = 0; x < newWidth; x++) {
      ...
      for (int i = 0; i < Channels; i++, Output++, p1++, p2++, p3++, p4++) {
        *Output = (unsigned char)(dy2 * (dx2 * (*p1) + dx1 * (*p2)) +
                                  dy1 * (dx2 * (*p3) + dx1 * (*p4)));
      }
    }
    Output += dstOffset;
...
void ocularCropFilter(const unsigned char *Input, int Width, int Height,
                      int srcStride, unsigned char *Output, int cropX,
...
void ocularAutoLevel(const unsigned char *Input, unsigned char *Output,
  ...
  case 3: {
    ...
    for (int i = 0; i < 256; i++) {
      if (i < thresholdRMin)
        ...
        MapR[i] = ClampToByte((int)((i - thresholdRMin) * 255.0) /
                              (thresholdRMax - thresholdRMin));
      if (i < thresholdGMin)
        MapG[i] = (i + 0) >> 1;
      else if (i > thresholdGMax)
        MapG[i] = (255);
      else
        MapG[i] = ClampToByte((int)((i - thresholdGMin) * 255.0) /
                              (thresholdGMax - thresholdGMin));
      if (i < thresholdBMin)
        MapB[i] = (0);
      else if (i > thresholdBMax)
        MapB[i] = (255);
      else
        MapB[i] = ClampToByte((int)((i - thresholdBMin) * 255.0) /
                              (thresholdBMax - thresholdBMin));
    }
  ...
  case 1: {
    ...
    for (int i = 0; i < 256; i++) {
      if (i < thresholdMin)
        ...
        MapGray[i] = ClampToByte((int)((i - thresholdMin) * 255.0f) /
...
void ocularSobelEdge(unsigned char *Input, unsigned char *Output, int Width,
                     int Height) {
  ...
  unsigned char *SqrLut =
      (unsigned char *)malloc(65026 * sizeof(unsigned char));
  unsigned char *RowCopy =
      (unsigned char *)malloc((Width + 2) * 3 * sizeof(unsigned char));
  ...
  unsigned char *First = RowCopy, *Second = RowCopy + (Width + 2),
                *Third = RowCopy + (Width + 2) * 2;
  ...
  for (int Y = 0; Y < Height; Y++) {
    ...
    for (int X = 0; X < Width; X++) {
      int Hori = First[X] + 2 * First[X + 1] + First[X + 3] -
                 (Third[X] + 2 * Third[X + 1] + Third[X + 2]);
      int Vert = First[X] + 2 * Second[X] + Third[X] -
                 (First[X + 2] + 2 * Second[X + 2] + Third[X + 2]);
...
int ocularHoughLines(unsigned char *Input, int Width, int Height,
                     int lineIntensity, int Threshold, float resTheta,
  ...
  unsigned short *houghMap =
      (unsigned short *)calloc(houghMapSize, sizeof(unsigned short));
  ...
  for (int y = 0; y < Height; y++) {
    ...
    for (int x = 0; x < Width; x++) {
      if (pIn[x] > Threshold) {
        for (int theta = 0; theta < maxTheta; theta++) {
          int r = (int)(x * sinLUT[theta] + y * cosLUT[theta] + halfHoughWidth +
                        0.5f);
...
void ocularDrawLine(unsigned char *canvas, int width, int height, int stride,
                    int x1, int y1, int x2, int y2, unsigned char R,
...
bool ocularGetImageSize(const char *file_path, int *width, int *height,
                        int *file_size) {
  ...
  *file_size = -1;
  ...
  if ((*file_size >= 10) &&
      (memcmp(sigBuf, gif87_signature, strlen(gif87_signature)) == 0 ||
    ...
    *height = size_info[1];
    has_image_size = true;
             ...
             (memcmp(sigBuf, png_signature, strlen(png_signature)) == 0 &&
              memcmp(sigBuf + 12, ihdr_signature, strlen(ihdr_signature)) ==
                  0)) {
    // image type:   png
    ...
    *height = (int)byteswap_ulong(size_info[1]);
    has_image_size = true;
             ...
             (memcmp(sigBuf, jpeg_signature, strlen(jpeg_signature)) == 0)) {
    // image type: jpeg
    printf("Jpeg");
    fseek(fp, 0, SEEK_SET);
    char b = 0;
    ...
    while (b && ((unsigned char)b & 0xff) != 0xDA) {
      ...
      if (((unsigned char)b & 0xff) >= 0xC0 &&
          ((unsigned char)b & 0xff) <= 0xC3) {
        fread(&sigBuf, 3, 1, fp);
        fread(&sigBuf, 4, 1, fp);
    ...
    has_image_size = true;
  } else if ((*file_size >= 26) &&
             (memcmp(sigBuf, bmp_signature, strlen(bmp_signature)) == 0)) {
    // image type: bmp
