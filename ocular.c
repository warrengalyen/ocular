...
#define max3(a, b, c) max(max((a), (b)), (c))
        ...
        if (Channels == 4) {
            ...
            for (int Y = 0; Y < Height; Y++) {
                ...
                for (int X = 0; X < Width; X++) {
                    ...
                    const unsigned char a = pInput[3];
                    outR =
                    ClampToByte(colorMatrix0[r] + colorMatrix1[g] + colorMatrix2[b] + colorMatrix3[a]);
                    unsigned char* pDegreeMapR = degreeMap + (r << 8);
                    pOutput[0] = pDegreeMapR[outR];
                    outG =
                    ClampToByte(colorMatrix4[r] + colorMatrix5[g] + colorMatrix6[b] + colorMatrix7[a]);
                    unsigned char* pDegreeMapG = degreeMap + (g << 8);
                    pOutput[1] = pDegreeMapG[outG];
                    outB =
                    ClampToByte(colorMatrix8[r] + colorMatrix9[g] + colorMatrix10[b] + colorMatrix11[a]);
    ...
    void ocularChromaKeyFilter(unsigned char* Input, unsigned char* Output, int Width, int Height,
        ...
        unsigned char maskY =
        (19589 * colorToReplaceR + 38443 * colorToReplaceG + 7504 * colorToReplaceB) >> 16;
    ...
    void ocularSaturationFilter(unsigned char* Input, unsigned char* Output, int Width, int Height,
        ...
        for (int Y = 0; Y < Height; Y++) {
            ...
            unsigned char* pInput = Input + (Y * Stride);
            for (int X = 0; X < Width; X++) {
                unsigned char* pSaturationMap = SaturationMap +
                (((13926 * pInput[0] + 46884 * pInput[1] + 4725 * pInput[2]) >> 16) << 8);
    ...
    // Photoshop-like levels adjustment. The min, max, minOut and maxOut parameters are floats in
    // the range[0, 1] .If you have parameters from Photoshop in the range[0, 255] you must first
    ...
    void rgb2yiq(unsigned char* R, unsigned char* G, unsigned char* B, short* Y, short* I, short* Q) {
        ...
        *Q =
        ((int)(0.211456f * 65536) * *R - (int)(0.522591f * 65536) * *G + (int)(0.311135f * 65536) * *B) >> 16;
    }
    ...
    void ocularMonochromeFilter(unsigned char* Input, unsigned char* Output, int Width, int Height,
        ...
        for (int luminance = 0; luminance < 256; luminance++) {
            ...
            (unsigned char)(255.0f *
                            (lum < 0.5f ? (2.0f * lum * fColorR) :
                                          (1.0f - 2.0f * (1.0f - lum) * (1.0f - fColorR))));
            filterColorGMap[luminance] =
            (unsigned char)(255.0f *
                            (lum < 0.5f ? (2.0f * lum * fColorG) :
                                          (1.0f - 2.0f * (1.0f - lum) * (1.0f - fColorG))));
            filterColorBMap[luminance] =
            (unsigned char)(255.0f *
                            (lum < 0.5f ? (2.0f * lum * fColorB) :
                                          (1.0f - 2.0f * (1.0f - lum) * (1.0f - fColorB))));
        }
        for (int Y = 0; Y < Height; Y++) {
            unsigned char* pOutput = Output + (Y * Stride);
            unsigned char* pInput = Input + (Y * Stride);
    ...
    // temperature : The temperature to adjust the image by, in ºK.A value of 4000 is very cool and
    // 7000 very warm. The default value is 5000. Note that the scale between 4000 and 5000 is
    // nearly as visually significant as that between 5000 and 7000. tint : The tint to adjust the
    // image by. A value of - 200 is very green and 200 is very pink. The default value is 0.
    ...
    // vibrance : The vibrance adjustment to apply, using 0.0 as the default, and a suggested min /
    // max of around - 1.2 and 1.2, respectively.
    ...
    void CalGaussianCoeff(float sigma, float* a0, float* a1, float* a2, float* a3, float* b1,
    ...
    void gaussianVertical(unsigned char* bufferPerLine, unsigned char* lpRowInitial,
                          unsigned char* lpColInitial, int height, int width, int Channels,
                          float a0a1, float a2a3, float b1b2, float cprev, float cnext) {
    ...
    void ocularGaussianBlurFilter(unsigned char* Input, unsigned char* Output, int Width,
        ...
        for (int x = 0; x < Width; ++x) {
            ...
            gaussianVertical(bufferPerLine, lpRowInitial, lpColInitial, Height, Width, Channels,
...
#define float2fixed(x) (((int)((x)*4096.0f + 0.5f)) << 8)
    ...
    void ocularUnsharpMaskFilter(unsigned char* Input, unsigned char* Output, int Width, int Height,
        ...
        case 3: {
            ...
            for (int Y = 0; Y < Height; Y++) {
                ...
                for (int X = 0; X < Width; X++) {
                    pTemp[0] =
                    (unsigned char)((19595 * pInput[0] + 38470 * pInput[1] + 7471 * pInput[2]) >> 16);
    ...
    // Radius : A radius in pixels to use for the blur, with a default of 2.0. This adjusts the
    // sigma variable in the Gaussian distribution function.
        ...
        case 3: {
            ...
            for (int i = 0; i < 256; i++) {
                if (i < thresholdRMin)
                    ...
                    MapR[i] =
                    ClampToByte((int)((i - thresholdRMin) * 255.0) / (thresholdRMax - thresholdRMin));
                if (i < thresholdGMin)
                    MapG[i] = (i + 0) >> 1;
                    ...
                    MapG[i] =
                    ClampToByte((int)((i - thresholdGMin) * 255.0) / (thresholdGMax - thresholdGMin));
                if (i < thresholdBMin)
                    MapB[i] = (0);
                    ...
                    MapB[i] =
                    ClampToByte((int)((i - thresholdBMin) * 255.0) / (thresholdBMax - thresholdBMin));
            }
        ...
        case 1: {
            ...
            for (int i = 0; i < 256; i++) {
                if (i < thresholdMin)
                    ...
                    MapGray[i] =
                    ClampToByte((int)((i - thresholdMin) * 255.0f) / (thresholdMax - thresholdMin));
            }
    ...
    void ocularSobelEdge(unsigned char* Input, unsigned char* Output, int Width, int Height) {
        ...
        for (int Y = 0; Y < Height; Y++) {
            ...
            for (int X = 0; X < Width; X++) {
                ...
                int Vert =
                First[X] + 2 * Second[X] + Third[X] - (First[X + 2] + 2 * Second[X + 2] + Third[X + 2]);
